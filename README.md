...
The `install-per-user.sh` script is meant to add an installation within the hierarchy of ~/.local, to make a JSON file with the text, `"filetype": "text/x-group-guess+json"` within the first 1, 000 bytes of the text, open with Group Guess by default.

It will register a new MIME type text/x-group-guess+json to make it easier to open Group Guess savefiles, and that change is if that line is present. If you want it to open in a text editor by default, simply don't add that property line. Group Guess itself doesn't make use of that JSON property (line) at all, it is only used for filetype associations. Alternatively, try using an extension `*.gg.json` or `*.ggjson`.
